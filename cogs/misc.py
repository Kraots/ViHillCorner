...
class CalculatorView(disnake.ui.View):
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction):
        if interaction.author.id != self.ctx.author.id:
            await interaction.response.send_message(f'Only {self.ctx.author.display_name} can use this calculator! If you wish to use it too please type `!calc`', ephemeral=True)
            return False
        return True
    ...
    async def on_timeout(self):
        ...
        if len(self.to_calc) != 0:
            return await self.message.edit(content='Timed Out.', embed=disnake.Embed(description=f'```py\n{eval(self.to_calc)}\n```', color=color.inviscolor), view=self)
        else:
            return await self.message.edit(embed=disnake.Embed(description=f'```\nTimed Out.\n```', color=color.inviscolor), view=self)
