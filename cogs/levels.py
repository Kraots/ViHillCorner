...
class LevelSystem(commands.Cog):
	...
	@commands.is_owner()
	async def _set(self, ctx, group : str = None, multiplier : int = None):
		if group == None:
			...
			if group in ['mod', 'staff', 'mods']:
				...
				await ctx.send("Set the multiplier for Mods/Staff members to **%s**." % (multiplier))
				return
				...
				await ctx.send("Set the multiplier for Server Boosters to **%s**." % (multiplier))
				return
			
			elif group in ['member', 'members']:
				await collection.update_one({'_id':374622847672254466}, {'$set':{'xp multiplier': multiplier}})
				await ctx.send("Set the multiplier for Members to **%s**." % (multiplier))
				return
