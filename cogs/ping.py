...
class Ping(commands.Cog):
    ...
    async def plping(self, ctx):
        debug("%s issued server command %s" % (str(ctx.message.author), str(ctx.message.content)))
        global up
        pongEmbed = discord.Embed(title="Pong!", description="_Pinging..._", color=0x2F3136)
        ...
        pongEmbed = discord.Embed(title="Pong!", description="Gateway: `%sms`\nRest: `%sms`" % (str(gateway_ping), str(int(round(end-start, 0)))), color=0x2F3136)
    ...
    async def uptime(self, ctx):
        uptime = discord.Embed(description=f"Bot has been online for: " + str(time_phaser(int(time.time()-up))), color=0x2F3136)
