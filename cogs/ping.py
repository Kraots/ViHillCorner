...
class Ping(commands.Cog):
    ...
    async def ping(self, ctx):
            ...
            botinfo = discord.Embed(title="Pong!", description=f"Websocket Latency: `{(round(self.bot.latency * 1000, 2))}ms`\nBot Latency: `{int(round(end-start, 0))}ms`\nResponse Time: `{(msg.created_at - ctx.message.created_at).total_seconds()/1000}` ms", color=color.lightpink)
            botinfo.set_footer(text=f"Online for {t.human_timedelta(dt=self.bot.uptime, suffix=False)}")
