...
class TicTacToe(disnake.ui.View):
    ...
    async def on_timeout(self):
        ...
        return await self.message.edit(
            content=f'The game has ended, **{self.turn.display_name}** took too much to react and lost **10,000** <:carrots:822122757654577183>.\n'
                    f'{winner.mention} won **10,000** <:carrots:822122757654577183>',
            view=self
        )
        ...
        if interaction.author.id != self.turn.id:
            if interaction.author.id not in (self.p1.id, self.p2.id):
                await interaction.response.send_message(
                    'You are not playing in this game! To start a game with someone you must have 10k carrots, if you do then type `!ttt <user>` to play.',
                    ephemeral=True
                )
                return False
    ...
    async def check_board_winner(self):
        ...
        if won is True:
            ...
            await self.message.edit(
                content=f'{self.turn.mention} won **10,000** <:carrots:822122757654577183>\n{loser.mention} lost **10,000** <:carrots:822122757654577183>',
                view=self
            )
            self.stop()
